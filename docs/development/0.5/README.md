# FerriteLib 0.5.x 团队执行与交付文档

本目录是 FerriteLib（下称 FL）0.5.x 的执行与交付面。台账规则不变：`AGENTS.md` 稳定、`MEMORY.md` 唯一易变账本、
`TODO.md` 行动面；本目录只承载**本轮的工作包、接口说明、消费者接入说明与证据表**，不另立竞争性状态源。

分支：`0.5.x`。基线：`c53bd37`（`0.4.x` / `origin/0.4.x` 同点）。契约轴本轮为 **0.5.0**。

## 阅读顺序

| 文件 | 内容 |
| --- | --- |
| [00-baseline.md](00-baseline.md) | 准确基线、取代关系、原版边界、API 窗口 |
| [10-work-packages.md](10-work-packages.md) | 工作包、依赖图、文件所有权、集成顺序、状态 |
| [20-api-and-xml.md](20-api-and-xml.md) | 公共 API / XML 使用说明与示例 |
| [30-consumer-handoff.md](30-consumer-handoff.md) | 面向消费者团队：版本区间、接入、迁移、已知限制 |
| [40-verification.md](40-verification.md) | 自动化验证结果、实机检查清单、未完成项 |
| [50-dev-package.md](50-dev-package.md) | 可用开发包位置、对应源码版本、构建命令 |

## 证据分级（全文强制）

任何一条陈述必须落在下列五类之一，禁止越过：

1. **已实现** —— 代码在树里，编译通过。
2. **已自动化验证** —— 本仓 harness/门禁在某个 revision 上跑绿，且写明该断言是"变更证明"还是"未来回归护栏"。
3. **已由真实消费者接入** —— 另一个仓库的代码在跑，需 `owner/repo@sha:path:line` 转录。
4. **已实机验证** —— 在真实游戏里被观察，需写明存档/路径/次数/观察者。
5. **尚待外部团队验证** —— 库侧已就绪，等待消费者或实机确认。

构建通过、打包成功、库内示例运行，**都不等于** 3 或 4。本仓自己的示例页永远不能抬高"已验证公共面"的计数
（`AGENTS.md` "Our own demo is not consumption"）。
## 当前状态（2026-09-15）

全部工作包已合并到 `0.5.x`：W0、P1(+P1h)、P2(+P2b/P2c)、P3、P4(+P4b/P4c)、P5，以及两处跨包缝合（task-13 壳的 fit
事件归属、task-14 通用页壳的诊断入口）。门禁与 harness 在每个合并提交后复跑；每次合并都经独立验证对抗式复核，
其报告在 [verification/](verification/README.md)。

**证据分级现状**：
- 已实现 + 已自动化验证：上述全部包（每个包都有突变证明与回归护栏两层，逐条见 [40-verification.md](40-verification.md)）。
- 已由真实消费者接入：**无**。本轮不修改消费者仓；接口与开发包已交付，接入由消费者团队执行。
- 已实机验证：**无**。本环境没有游戏会话；A1–A11 清单在 [40-verification.md](40-verification.md) §3。
- 尚待外部团队验证：全部实机行为与真实页面接入。
## ⚠ 交付结论更正（2026-09-15，外部 review 之后）

外部独立 review 对 tip `0117c01` 判 **Request changes**，并复现了 7 个库侧缺陷（R1–R7）。Lead 已在本机逐行复现其探针输出。
因此：**包已合并、门禁绿**仍然成立，但**「只剩外部验收」不成立**——修复完成并通过独立复验之前，本轮不是可交付状态。
缺陷清单、复现证据与处置见 [40-verification.md](40-verification.md) §7。
## 修复与复验状态（2026-09-16）

R1–R7 已全部修复并合入 `0.5.x`；外部探针在本机与独立复验的干净提取上复跑，七行输出全部转为已修复形态；
门禁 10/10、harness ok=1515 / 0 FAIL、隐私与中立性干净。
逐条证据与仍未关闭的验证项见 [40-verification.md](40-verification.md) §7 与 `verification/post-fix-verification.md`。
**仍然不构成交付验收**：实机 A1–A11 与真实消费者编译两项外部验收未被任何人完成。
