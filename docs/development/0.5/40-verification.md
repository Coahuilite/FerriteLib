...
## 6. 本轮对失败阶梯的一处刻意收窄（Lead 裁定，2026-09-15）

失败阶梯原口径（`MEMORY.md` / 主计划 §4）：**结构 fail-closed，外观值 fail-soft 且不得静默**。

P4c 把**页面级**默认值（`DefaultScheme`/`DefaultDensity` 指向本文件未声明的名字）从"软丢弃 + 记录"改为
"**拒绝该候选版本，last-known-good 继续生效**"，理由是它同时满足两件事：让"候选先校验"这条契约真的可被证伪
（独立验证发现原先的 style 预校验分支可以被整体掏空而 suite 全绿），并且避免页面以作者从未写过的外观"成功提交"。

**裁定：保留收窄**，同时把代价与边界写在这里，避免它被当成全局规则：

- 只在**页面级**默认值上收窄；**逐元素**未知 `Tone`/`Emphasis` 仍为 fail-soft（回退默认表现并记录），不变。
- 代价：该文档里其余合法的 scheme/density 在这一版里也不生效，改对后一起生效（见 `30-consumer-handoff.md` §4）。
- 首次加载没有 last-known-good 时的回退目标（内嵌布局 / 空样式文档）由独立验证实测登记，不在此处推测。

若后续维护者认为"逐项 fail-soft、只丢那一个名字"更合适，恢复路径是明确的：把页面级名字解析改回软丢弃，
并改用**已存在的结构级失败**（`UiStyleDocument.StructurallyInvalid`）来驱动 style 预校验 lane 的非空证明。
