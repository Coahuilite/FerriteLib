...
**追加第二部分（verifier，`e39acb0`）**：R1 的 rollback 半边与 **R2–R6 的植入旧缺陷测量已由独立复验完成**（各自 lane 与原始红行数：R1 3 / R2 5 / R3 3 / R4 5 / R5 2 / R6 4）。
因此 R1–R6 的记法升级为：**已修复合入 + 仓内护栏 + 独立测量的植入红**。

**R7 被第二次、更强的证伪**：委派的复验者植入了一个**真实的**第二次读盘（`Parse(Decode(File.ReadAllBytes(path)))`，大小与版本仍取第一次的 bytes），
该 lane **保持全绿**（1515 ok / 0 FAIL）。所以这条护栏是**词法**的、双向都不可靠：加一行注释能让它变红，真正的 TOCTOU 回归能让它变绿。
